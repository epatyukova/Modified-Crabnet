...
# %%
def BCEWithLogitsLoss(output, log_std, target, weight=None):
    # need to add mask
    mask=(~target.isnan())
    target_nonan=torch.nan_to_num(target, nan=0.0)
    loss = nn.functional.binary_cross_entropy_with_logits(output, target_nonan, weight=weight, reduction='none')
    loss=loss*mask.long()
    return loss.sum()/mask.long().sum()
...
def RobustL1(output, log_std, target,mask=None):
    """
    Robust L1 loss using a lorentzian prior. Allows for estimation
    of an aleatoric uncertainty.
    """
    mask=(~target.isnan())
    # need to correct futher..
    target_nonan=torch.nan_to_num(target, nan=0.0)
    absolute = torch.abs(mask.long()*(output - target_nonan))
    log_std=mask.long()*log_std
...
class CombinedScaler():
    def __init__(self, data, scale_map):
        self.mask=np.isnan(data)
        ma_data=np.ma.array(data, mask=self.mask)
        self.data = torch.as_tensor(ma_data, dtype=data_type_torch)
        self.mean = torch.mean(self.data,axis=1)
        self.std = torch.std(self.data,axis=1)
        self.scale_map=scale_map

    def scale(self, data):
        data = torch.as_tensor(data)
        data_scaled=torch.zeros_like(data)
        for i in range(len(self.scale_map)):
            if(self.scale_map[i]==True):
                data_scaled[i,:] = (data[i,:] - self.mean[i]) / self.std[i]
            else:
    ...
    def unscale(self, data_scaled):
        data_scaled = torch.as_tensor(data_scaled)
        data = torch.as_tensor(data_scaled)
        for i in range(len(self.scale_map)):
            if(self.scale_map[i]==True):
                data[i,:] = data_scaled[i,:] * self.std[i] - self.mean[i]
            else:
                data[i,:] = data_scaled[i,:]
        return data
...
class EDMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        X = self.X[idx, :, :]
        y = self.y[:,idx]  # change from self.y[idx] to accomodate multiple properties
...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    energy = df['form_energy_per_atom'].values.astype(data_type_np)
    stability = df['stable'].values.astype(data_type_np)
    hull=df['energy_above_hull'].values.astype(data_type_np)
    disorder=df['disorder'].values.astype(data_type_np)

    return out, [energy, stability, hull, disorder], formula
