...
class Embedder(nn.Module):
    def __init__(self,
        ...
        mat2vec = 'data/element_properties/mat2vec.csv'  # element embedding
    ...
    def forward(self, src):
        mat2vec_emb = self.cbfv(src.type(torch.long))
...
class CrabNet_Multi(nn.Module):
    def __init__(self,
        ...
        if residual_nn == 'roost':
            ...
            self.output_nn_energy = ResidualNetwork(self.d_model,
                                             self.out_dims,
                                             self.out_hidden)
            self.output_nn_stability = ResidualNetwork(self.d_model,
                                             self.out_dims,
                                             self.out_hidden)
            self.output_nn_disorder = ResidualNetwork(self.d_model,
                                             self.out_dims,
                                             self.out_hidden)
            self.output_nn_hull = ResidualNetwork(self.d_model,
                                             self.out_dims,
                                             self.out_hidden)
    ...
    def forward(self, src, frac):
        ...
        output_energy = self.output_nn_energy(output)  # simple linear
        output_stability = self.output_nn_stability(output)
        output_hull = self.output_nn_hull(output)
        output_disorder = self.output_nn_disorder(output)
        if self.avg:
            result=[]
            for output_i in [output_energy, output_stability, output_hull, output_disorder]:
                output_i = output_i.masked_fill(mask, 0)
                output_i = output_i.sum(dim=1)/(~mask).sum(dim=1)
                output_i, logits_i = output_i.chunk(2, dim=-1)
                probability_i= torch.ones_like(output_i)
                probability_i[:, :logits_i.shape[-1]] = torch.sigmoid(logits_i)
                output_i = output_i * probability_i
