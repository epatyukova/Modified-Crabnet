...
from utils.utils import masked_balanced_accuracy, compound_accuracy
...
# %%
def get_model(data_dir, mat_prop, classification=False, multiclass=False, batch_size=None,
    ...
    train_data = f'{data_dir}/{mat_prop}/train.json'
    try:
        val_data = f'{data_dir}/{mat_prop}/val.json'
    except:
    ...
    data_size = pd.read_json(train_data).shape[0]
...
def load_model(data_dir, mat_prop, classification, multiclass,file_name, verbose=True):
...
def save_results(data_dir, mat_prop, classification, file_name, verbose=True):
    ...
    if model.classification:
        ...
        acc_el=masked_balanced_accuracy(output[0], output[1],output[4])
        acc_c,acc_o,acc_d=compound_accuracy(output[0], output[1],output[4])
        print(f'{mat_prop} Element acc: {acc_el:0.3f}')
        print(f'{mat_prop} Compount acc: {acc_c:0.3f}')
        print(f'{mat_prop} Ordered compount acc: {acc_o:0.3f}')
        print(f'{mat_prop} Disordered compount acc: {acc_d:0.3f}')
    else:
...
if __name__ == '__main__':
    ...
    if train:
        ...
        model = get_model(data_dir, mat_prop, classification, multiclass, verbose=True)
