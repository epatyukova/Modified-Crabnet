...
def CrossEntropyLoss(output, target, mask, weight=None):
    loss = nn.functional.cross_entropy(output.transpose(1,2), target.transpose(1,2), weight=weight,reduction='none')
    mask_reduced=mask.mean(dim=2)
    loss=loss*mask_reduced
    #loss = loss.sum(dim=1)/mask_reduced.sum(dim=1)
    return loss.sum()/mask_reduced.sum()
...
def masked_balanced_accuracy(pred, act, mask_data):
    # according to sklearn balanced accuracy is defined as average recall defined for each class
    # recall = true positive / all positive

    # we assume here 3 classes
    # this accuracy is elementwise, we need to define another one for whole compound
    predictions = np.argmax(pred,axis=2).reshape(-1)
    correct = np.argmax(act,axis=2).reshape(-1)
    mask_data = np.mean (mask_data,axis=2).reshape(-1)
    mask_data=(mask_data==0)
    masked_predictions=np.ma.array(predictions,mask=mask_data)
    masked_correct = np.ma.array(correct,mask=mask_data)
    scores=np.zeros(3)
    zero_classes=0
    for i in range(3):
        total=len(masked_correct[masked_correct==i])
        if(total>0):
            y=masked_predictions[masked_correct==i]
            scores[i]=len(y[y==i])/total
        else:
            zero_classes+=1
    return np.sum(scores)/(3.0-zero_classes)

def compound_accuracy(pred, act, mask_data):
    predictions = np.argmax(pred,axis=2)
    correct = np.argmax(act,axis=2)
    ...
    for i in range(len(masked_predictions)):
        if(np.all(masked_predictions[i]==masked_correct[i])):
            compound_class.append(1)
        else:
            compound_class.append(0)
        if(np.all(masked_correct[i]==0)):
            if(np.all(masked_predictions[i]==masked_correct[i])):
                compounds_class_ord.append(1)
            else:
                compounds_class_ord.append(0)
        else:
            if(np.all(masked_predictions[i]==masked_correct[i])):
                compounds_class_dis.append(1)
            else:
                compounds_class_dis.append(0)
    if(len(compound_class)>0):
        acc=np.sum(compound_class)/len(compound_class)
    else:
        acc=0
    if(len(compounds_class_ord)>0):
        acc_ord=np.sum(compounds_class_ord)/len(compounds_class_ord)
    else:
        acc_ord=0
    if(len(compounds_class_dis)>0): 
        acc_dis=np.sum(compounds_class_dis)/len(compounds_class_dis)
    else:
        acc_dis=0
    return acc, acc_ord, acc_dis
...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    if drop_unary:
        df = df.loc[df['count'] != 1]  # drop pure elements
    # if not inference:
    ...
    if multiclass==False:
        ...
        y=np.zeros((len(df),n_elements,3))
        for i in range(len(df)):
            for j,x in enumerate(elem_num[i]):
                if(x!=0):
                    cls=df.element_disorder[i][all_symbols[int(x)-1]]
                    y[i,j,cls]=1
